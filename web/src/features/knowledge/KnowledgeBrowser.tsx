...
export function KnowledgeBrowser({ pages, selectedSlug, warnings, onSelect, onOpen }: { pages: KnowledgePage[]; selectedSlug?: string; warnings: KnowledgeWarning[]; onSelect: (slug: string) => void; onOpen: (slug: string) => void }) {
	...
	return <div className="knowledge-browser">
		...
		<section className="knowledge-summary" aria-label="Selected page summary"><div className="knowledge-summary-card">{selected ? <><p className="knowledge-eyebrow">{selected.domain} · {selected.pageType || 'PAGE'}</p><h2>{selected.title}</h2><p>{selected.summary || 'No summary provided.'}</p><div className="knowledge-tags">{selected.roles.map((role) => <span key={`role-${role}`}>{role}</span>)}{selected.topics.map((topic) => <span key={`topic-${topic}`}>{topic}</span>)}</div><button type="button" className="primary" onClick={() => onOpen(selected.slug)}>Read page</button></> : <><h2>Select a page</h2><p>Choose a page to inspect its summary and metadata.</p></>}</div><KnowledgeWarnings warnings={warnings} compact /></section>
	</div>;
}
