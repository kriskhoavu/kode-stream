...
export function EmbeddedTerminal({ initial, visible, mode, title, subtitle, onStartMove, onToggleDockMode, onToggleMinimize, onToggleMaximize, onClose, onStateChange, cancelOnClose = true, compact = false }: { initial: EmbeddedAISessionResult; visible: boolean; mode: 'floating' | 'side_panel' | 'maximized'; title: string; subtitle: string; onStartMove: (x: number, y: number) => void; onToggleDockMode: () => void; onToggleMinimize: () => void; onToggleMaximize: () => void; onClose: () => void; onStateChange?: (state: EmbeddedAISessionState, exitCode?: number) => void; cancelOnClose?: boolean; compact?: boolean }) {
	...
	return <section className={`embedded-terminal-panel${visible ? ' active' : ''}${compact ? ' embedded-terminal-compact' : ''}`} aria-hidden={!visible} role={visible ? 'dialog' : undefined} aria-modal={visible && !compact ? 'true' : undefined} aria-labelledby={visible ? `embedded-terminal-title-${initial.session.id}` : undefined}>
		<header onPointerDown={startMove}><div className="embedded-terminal-heading"><span className="embedded-terminal-heading-icon"><SquareTerminal size={19} /></span><div><h2 id={`embedded-terminal-title-${initial.session.id}`}>{title}</h2><p>{subtitle} · <span role="status" aria-live="polite">{stateLabel(state, connection, exitCode)}</span></p></div></div><div className="embedded-terminal-window-actions">{!compact && <><button ref={controlsRef} className="icon-button" type="button" title="Minimize" aria-label="Minimize embedded terminal" onClick={onToggleMinimize}><Minus size={18} strokeWidth={1.8} /></button><button className="icon-button" type="button" title={mode === 'side_panel' ? 'Float window' : 'Dock to right side'} aria-label={mode === 'side_panel' ? 'Float embedded terminal window' : 'Dock embedded terminal to right side'} onClick={onToggleDockMode}>{mode === 'side_panel' ? <Columns2 size={16} strokeWidth={1.8} /> : <PanelRight size={16} strokeWidth={1.8} />}</button><button className="icon-button" type="button" title={mode === 'maximized' ? 'Restore' : 'Maximize'} aria-label={mode === 'maximized' ? 'Restore embedded terminal size' : 'Maximize embedded terminal'} onClick={onToggleMaximize}>{mode === 'maximized' ? <Copy size={16} strokeWidth={1.8} /> : <Square size={15} strokeWidth={1.8} />}</button></>}<button ref={compact ? controlsRef : undefined} className="icon-button embedded-terminal-close" type="button" title={compact ? 'Hide terminal' : 'Close'} aria-label={compact ? 'Hide terminal' : 'Close embedded terminal'} onClick={close}><X size={18} strokeWidth={1.8} /></button></div></header>
