...
export function EmbeddedTerminal({ initial, visible, mode, title, onToggleMinimize, onToggleMaximize, onClose }: { initial: EmbeddedAISessionResult; visible: boolean; mode: 'normal' | 'maximized' | 'minimized'; title: string; onToggleMinimize: () => void; onToggleMaximize: () => void; onClose: () => void }) {
	...
	return <section className={`embedded-terminal-panel${visible ? ' active' : ''}`} aria-hidden={!visible} role={visible ? 'dialog' : undefined} aria-modal={visible && mode !== 'minimized' ? 'true' : undefined} aria-labelledby={visible ? `embedded-terminal-title-${initial.session.id}` : undefined}>
		<header><div><h2 id={`embedded-terminal-title-${initial.session.id}`}>{title}</h2><p role="status" aria-live="polite">{stateLabel(state, connection, exitCode)}</p></div><div className="embedded-terminal-window-actions"><button className="icon-button" type="button" aria-label={mode === 'minimized' ? 'Restore embedded terminal' : 'Minimize embedded terminal'} onClick={onToggleMinimize}>{mode === 'minimized' ? <Maximize2 size={18} /> : <Minimize2 size={18} />}</button><button className="icon-button" type="button" aria-label={mode === 'maximized' ? 'Restore embedded terminal size' : 'Maximize embedded terminal'} onClick={onToggleMaximize}>{mode === 'maximized' ? <Shrink size={18} /> : <Maximize2 size={18} />}</button><button className="icon-button" type="button" aria-label="Close embedded terminal" onClick={close}><X size={18} /></button></div></header>
