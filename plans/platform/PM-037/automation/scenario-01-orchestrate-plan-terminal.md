...
| 2   | Choose **Launch terminal** using the default provider configured in Settings.         | **Launching…** clears and a terminal or contextual error appears. | One durable session is created. A successful launch exposes one live terminal and one session node or unplaced session item linked to the plan. | Safe write  |
