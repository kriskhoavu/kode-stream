...
```mermaid
flowchart TD
  L[Local application] --> D[datadir: YAML/JSONL]
  L --> S[database: SQLite]
  C[Cloud control plane] --> P[database: Postgres]
  P --> A[Agent-Backed workspace metadata]
  P --> R[Remote Snapshot workspace metadata]
```

Storage does not choose how repository work executes. Local storage choices apply only to a local application. Both
