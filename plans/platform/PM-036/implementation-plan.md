...
- [x] Add E2E runbook and latest-result models without persistence.
- [x] Discover eligible plan runbooks and source-reference-matched canonical pages.
- [x] Parse safe `automation/results/latest.md` files and expose plan/Knowledge read routes.
- [x] Add focused service and API tests for ordering, missing results, malformed content, and unsafe paths.
...
- [x] Add validated workspace-context external and embedded session routes.
- [x] Reuse provider capability discovery with workspace scope.
- [x] Preserve all item-context launch behavior and tests.
...
- [x] Add E2E runbook and latest-result frontend types plus API client methods.
- [x] Build `E2EQualityPanel` with cards, result metadata, diagnostics, and refresh.
- [x] Add constrained E2E mode to the AI Session dialog/control.

**Verification:** `npm test -- E2EQualityPanel AISessionLaunchDialog`
...
- [x] Integrate the panel below the existing plan Quality controls.
- [x] Integrate the panel into Knowledge Reader only for E2E pages.
- [x] Connect workspace-context launch and result refresh behavior.

**Verification:** `npm test -- ItemWorkspacePage KnowledgePage KnowledgeReader`
