...
describe('KnowledgeBrowser', () => {
	it('groups pages, filters metadata, and renders warnings', () => {
		render(<KnowledgeBrowser pages={pages} selectedSlug="offer-overview" warnings={[{ slug: 'offer-overview', code: 'unresolved_link', message: 'Missing target' }]} onSelect={vi.fn()} onOpen={vi.fn()} />);
	...
	it('supports keyboard movement and activation', () => {
		const onOpen = vi.fn();
		render(<KnowledgeBrowser pages={pages} warnings={[]} onSelect={vi.fn()} onOpen={onOpen} />);
		...
		fireEvent.keyDown(second, { key: 'Enter' }); expect(onOpen).toHaveBeenCalledWith('article-import');
	});

	it('explains an empty valid Wiki', () => {
		render(<KnowledgeBrowser pages={[]} warnings={[]} onSelect={vi.fn()} onOpen={vi.fn()} />);
