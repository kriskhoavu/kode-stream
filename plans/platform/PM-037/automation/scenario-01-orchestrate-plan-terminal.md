...
| 4   | Select the workspace node, then return to the session node.                           | The Workbench opens, then the session expands inside the Canvas. | The same process remains active, the right Workbench stays closed for the session, and returning does not relaunch it.                | Read-only   |
