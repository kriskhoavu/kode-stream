...
export function KnowledgePage({ workspaces, location, onLocationChange, onOpenExplorer }: { workspaces: WorkspaceConfig[]; location?: KnowledgeLocation; onLocationChange: (location: KnowledgeLocation) => void; onOpenExplorer: (workspaceId: string, path: string) => void }) {
	...
	return <section className="knowledge-page">
		<header className="knowledge-header">
			...
			<div className="knowledge-views" aria-label="Knowledge view">{(['browse', 'read', 'graph'] as const).map((view) => <button type="button" key={view} className={(location?.view ?? 'browse') === view ? 'active' : ''} disabled={view !== 'browse' && !controller.page} onClick={() => controller.updateLocation({ view })}>{view[0].toUpperCase() + view.slice(1)}</button>)}</div>
