...
| Navigation guard   | Terminal component      | Warns before browser unload while sessions are active |
...
- Provide confirmed close as the single cancellation path for a running session.
...
- Fully hide the dock while minimized and leave only a compact bottom-right restore chip.
