...
vi.mock('../features/canvas/useCanvasState', () => ({ useCanvasState: () => ({ projection: canvasState.projection, loading: false, error: '', conflicts: [], dirtyCount: 0, hasUnsavedChanges: false, moveNode: vi.fn(), saveViewport: vi.fn(), reloadPosition: vi.fn(), reapplyPosition: vi.fn(), placeUnplaced: vi.fn(), removeNode: vi.fn(), resetPositions: vi.fn(), reload: vi.fn(), refresh: vi.fn() }) }));
...
describe('CanvasPage', () => {
	...
	it('keeps branch context explicit and exposes a polite save-status region', () => {
		canvasState.projection = projection();
		render(<CanvasPage workspace={workspace} location={{ workspaceId: workspace.id, branch: 'main' }} onLocationChange={vi.fn()} />);
		expect(screen.getByRole('combobox', { name: 'Canvas branch' })).toHaveValue('main');
