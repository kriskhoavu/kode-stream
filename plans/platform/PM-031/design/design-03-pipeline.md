...
PM-031 used a gated migration pipeline. Each route family passed focused parity checks before the final cutover removed the legacy fallback.
...
| Route inventory update  | Family status is recorded as Gin-owned.                               |
...
| Family-by-family migration | Small review units, clear rollback, focused tests | More phases and temporary dual-stack complexity.           |
| Keep fallback permanently  | Lowest immediate risk                             | Leaves duplicate routing and unfinished migration.         |

## Selected Approach

Family-by-family migration with explicit cutover. This matches PM-030 and avoided a broad rewrite while still ending in a Gin-only API.
...
| Governance tests      | Fail if API `ServeMux` registrations or import boundaries regress. |
