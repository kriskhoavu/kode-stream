...
func TestCheckReturnsHealthyWorkspace(t *testing.T) {
	...
	service := New(workspaceStub{workspace}, itemStub{}, gitStub{root: root})
...
func TestCheckReturnsWarningsForRecoverableState(t *testing.T) {
	...
	service := New(workspaceStub{workspace}, itemStub{}, gitStub{root: root, branchErr: errors.New("missing"), status: models.GitStatus{Conflicted: true}})
...
func TestCheckReturnsFailedForMissingWorkspacePath(t *testing.T) {
	...
	workspace := models.WorkspaceConfig{ID: "workspace", Path: root, Sources: []string{"plans"}, BaselineBranch: "main", LastScannedAt: time.Now()}
	service := New(workspaceStub{workspace}, itemStub{err: errors.New("corrupt")}, gitStub{root: root, rootErr: errors.New("not git")})
