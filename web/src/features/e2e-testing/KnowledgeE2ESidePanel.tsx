...
export function KnowledgeE2ESidePanel({ workspaceId, root, detail }: { workspaceId: string; root: string; detail: KnowledgePageDetail }) {
	...
	return <>
		<div className="side-panel-tabs knowledge-side-panel-tabs" role="tablist" aria-label="Knowledge side panel"><button type="button" className="active" aria-selected="true"><GitCompare size={14} /> Quality</button></div>
		<E2EQualityPanel workspaceId={workspaceId} runbooks={e2e.runbooks} diagnostic={e2e.diagnostic} onRefresh={refresh} />
	</>;
}
