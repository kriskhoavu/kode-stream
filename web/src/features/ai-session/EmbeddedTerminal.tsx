...
export function EmbeddedTerminal({ initial, onClose }: { initial: EmbeddedAISessionResult; onClose: () => void }) {
	...
	useEffect(() => {
		...
		void Promise.all([import('@xterm/xterm'), import('@xterm/addon-fit')]).then(([terminalModule, fitModule]) => {
			...
			const fit = new fitModule.FitAddon(); terminal.loadAddon(fit); terminal.open(hostRef.current); fit.fit(); terminal.focus(); terminalRef.current = terminal;
		...
		return () => { disposed = true; if (retry) window.clearTimeout(retry); observer?.disconnect(); dataDisposable?.dispose(); resizeDisposable?.dispose(); socketRef.current?.close(); terminal?.dispose(); terminalRef.current = null; };
	}, [initial]);
	...
	return <div className="modal-backdrop embedded-terminal-backdrop" role="presentation"><section className="embedded-terminal-panel" role="dialog" aria-modal="true" aria-labelledby="embedded-terminal-title">
		<header><div><h2 id="embedded-terminal-title">Embedded {initial.session.provider} session</h2><p role="status" aria-live="polite">{stateLabel(state, connection, exitCode)}</p></div><button className="icon-button" type="button" aria-label="Close embedded terminal" onClick={close}><X size={18} /></button></header>
