...
| E2E dialog mode              | Lock runbook context, generated prompt, and selected `e2e-testing` skill; permit provider, terminal, and surface selection. |

Each runbook card shows title, source badge, workspace-relative path, latest status, provider/environment when present, failed step for failed or blocked runs, and evidence links that can be opened with existing workspace file/path actions.
