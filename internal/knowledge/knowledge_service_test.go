...
func (d *stubDetector) DetectWorkspace(_ context.Context, _ models.WorkspaceConfig) ([]KnowledgeWiki, error) {
	d.calls++
	d.workspaceCalls++
	return d.wikis, d.err
}

func (d *stubDetector) DetectSource(_ context.Context, _ models.WorkspaceConfig, source string) (KnowledgeWiki, bool, error) {
	d.calls++
	d.sourceCalls++
	if d.err != nil {
		return KnowledgeWiki{}, false, d.err
	}
...
func TestDisabledKnowledgeRejectsActionsAndHidesPersistedWikis(t *testing.T) {
	...
	if err := store.ReplaceWorkspace("ws", []KnowledgeWiki{{Root: "docs", Pages: []KnowledgePage{}, Warnings: []KnowledgeWarning{}}}); err != nil {
		t.Fatal(err)
	}
	service := NewService(registry.New(registryPath, gitadapter.New()), store)
...
func TestEnrichPassesLiteralArgumentsRescansAndBoundsOutput(t *testing.T) {
	...
	detector, audits := &stubDetector{wikis: []KnowledgeWiki{{Root: "docs", Pages: []KnowledgePage{}, Warnings: []KnowledgeWarning{}}}}, &stubAudit{}
...
func newKnowledgeService(t *testing.T) (*KnowledgeService, string) {
	...
	store := NewStore(filepath.Join(directory, "knowledge-index.yaml"))
	pages := []KnowledgePage{
		{Slug: "index", Title: "Index", Path: "index.md", Domain: "root", Roles: []string{}, Topics: []string{}, Links: []KnowledgeLink{{SourceSlug: "index", RawTarget: "guide", Resolution: LinkResolved, TargetSlug: "guide"}}, Backlinks: []string{}},
		{Slug: "guide", Title: "Guide", Path: "guide.md", Domain: "root", Roles: []string{}, Topics: []string{}, Links: []KnowledgeLink{}, Backlinks: []string{"index"}},
	}
	if err := store.ReplaceWorkspace("ws", []KnowledgeWiki{{Root: "docs", DisplayName: "Docs", Pages: pages, Warnings: []KnowledgeWarning{}}}); err != nil {
		t.Fatal(err)
	}
	return NewService(reg, store), "ws"
}

func newActionService(t *testing.T, executable string, args []string) (*KnowledgeService, *Store) {
	t.Helper()
	return newActionServiceAt(t, t.TempDir(), executable, args)
}

func newActionServiceAt(t *testing.T, workspaceRoot, executable string, args []string) (*KnowledgeService, *Store) {
	t.Helper()
	directory := t.TempDir()
	...
	store := NewStore(filepath.Join(directory, "knowledge-index.yaml"))
	if err := store.ReplaceWorkspace("ws", []KnowledgeWiki{{Root: "old", Pages: []KnowledgePage{}, Warnings: []KnowledgeWarning{}}}); err != nil {
		t.Fatal(err)
	}
	return NewService(registry.New(registryPath, gitadapter.New()), store), store
}
