	...
	return (
		<section className="canvas-page" aria-label="Workspace Canvas">
			...
			{canvas.projection && <div className="canvas-state canvas-status-strip" data-testid="canvas-ready">
				<strong>{canvas.projection.nodes.length} placed</strong><span>{canvas.projection.unplaced.length} new</span><span role="status" aria-live="polite">{canvas.saveStatus === 'saving' ? `Saving ${canvas.dirtyCount} position${canvas.dirtyCount === 1 ? '' : 's'}…` : canvas.saveStatus === 'saved' ? 'Saved' : ''}</span>
			</div>}
