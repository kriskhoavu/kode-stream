...
export function KnowledgeActions({ workspaceId, settings, root, busy, onRun }: { workspaceId: string; settings?: KnowledgeSettings; root?: string; busy: boolean; onRun: (operation: 'rescan' | 'sync' | 'enrich', confirm?: boolean) => Promise<unknown> }) {
	...
	return <div className="knowledge-actions"><button type="button" className="secondary" disabled={busy || disabled || !root} onClick={() => void onRun('rescan')}><RotateCw size={14} /> Sync</button><button type="button" className="secondary" disabled={busy || disabled} onClick={() => void sync()}><GitPullRequest size={14} /> Pull</button><button type="button" className="secondary" disabled={busy || disabled || !settings?.enrichExecutable} title={settings?.enrichExecutable ? 'Run configured enrichment' : 'Configure enrichment in Workspace settings'} onClick={() => void enrich()}><Sparkles size={14} /> Enrich</button></div>;
}
