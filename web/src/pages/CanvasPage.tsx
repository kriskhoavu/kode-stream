...
export function CanvasPage({ workspace, location, onLocationChange }: { workspace?: WorkspaceConfig; location?: CanvasLocation; onLocationChange: (location: CanvasLocation) => void }) {
	...
	return (
		<section className="canvas-page" aria-label="Workspace Canvas">
			...
			{canvas.projection && <div className={`canvas-work-area${selectedNode ? ' workbench-open' : ''}`}><CanvasBoard projection={canvas.projection} conflicts={canvas.conflicts} selectedId={selectedId} onSelect={setSelectedId} onMoveNode={canvas.moveNode} onSaveViewport={canvas.saveViewport} onReloadPosition={(id) => void canvas.reloadPosition(id)} onReapplyPosition={(id) => void canvas.reapplyPosition(id)} onPlaceUnplaced={() => void canvas.placeUnplaced()} onReset={() => setConfirmation('reset')} onRemove={(node) => { setSelectedId(node.id); setConfirmation('remove'); }} /><CanvasWorkbench projection={canvas.projection} selectedNode={selectedNode} onClose={closeWorkbench} onReload={canvas.refresh} onSelectNode={setSelectedId} onPlaceUnplaced={canvas.placeUnplaced} /></div>}
